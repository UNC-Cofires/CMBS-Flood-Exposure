...
outfolder = os.path.join(pwd,'treatment_status')
os.makedirs(outfolder,exist_ok=True)

# Get command-line arguments
claim_rate_path = sys.argv[1]
geog_unit = sys.argv[2]
neighbor_graph = sys.argv[3]
claim_rate_threshold = float(sys.argv[4])
treatment_duration = int(sys.argv[5])
scenario_name = sys.argv[6]
...
# Load data on claim rate by geographic unit
claim_rate = pd.read_parquet(claim_rate_path)
claim_rate['claimRate'] = claim_rate['claimRate'].fillna(0)

# Load data on spatial neighbor relationships between geographic units
neighboring_units = pd.read_parquet(config['paths'][neighbor_graph])

### *** EVALUATE TREATMENT STATUS OF FLOOD-EXPOSED UNITS *** ###

# Define an "event" as any year in which an area's claim rate exceeds a set threshold
...
treatment_status = claim_rate.groupby(geog_unit).apply(lambda x: evaluate_treatment_status(x['flood_event'],x['year'],treatment_duration))
treatment_status = treatment_status.reset_index()
treatment_status['treatment_description'] = treatment_description

### *** EVALUATE SPILLOVER STATUS OF NEIGHBORS OF FLOOD-EXPOSED UNITS *** ###

# Represent neighbor relationships between units as undirected graph
neighbor_graph = nx.Graph()
neighbor_graph.add_nodes_from(treatment_status[geog_unit].unique())
neighbor_graph.add_edges_from(neighboring_units.to_numpy())

# Get list of flood events by geographic unit / time
flood_events = treatment_status[['calendar_time',geog_unit,'treatment_event']]
flood_events = flood_events[flood_events['treatment_event']==1].rename(columns={'calendar_time':'year'})

# Determine which units were neighbors to a flooded unit
flood_events['neighbors'] = flood_events[geog_unit].apply(lambda x: list(neighbor_graph.neighbors(x)))
neighbor_events = flood_events[['year','neighbors']].explode('neighbors').drop_duplicates()
neighbor_events = neighbor_events.rename(columns={'neighbors':geog_unit,'calendar_time':'year'})
neighbor_events['neighbor_flood_event'] = 1

# Exclude entries where the unit itself was also flooded
neighbor_events = pd.merge(neighbor_events,flood_events.drop(columns=['neighbors']),on=[geog_unit,'year'],how='left').fillna(0)
...
neighbor_events = pd.merge(claim_rate[[geog_unit,'year']],neighbor_events,on=[geog_unit,'year'],how='left').fillna(0)
neighbor_events['neighbor_flood_event'] = neighbor_events['neighbor_flood_event'].astype(int)

# Evaluate spillover status over time for neighbors of flooded units
spillover_status = neighbor_events.groupby(geog_unit).apply(lambda x: evaluate_treatment_status(x['neighbor_flood_event'],x['year'],treatment_duration))
...
treatment_status = pd.merge(treatment_status,spillover_status,on=[geog_unit,'calendar_time'],how='left')
